...
fetch('https://rppicalc.onrender.com/IndicesbyRegionhouse')
    ...
    y.domain([0, d3.max(data1, (d) => d.Gasabo)]);

    // Plot house bars and labels
    svg
        .selectAll(".bar.house-gasabo")
        .data(data1)
        .enter()
        .append("rect")
        .attr("class", "bar house-gasabo")
        ...
        .duration(1000)
        .attr("y", (d) => y(d.Gasabo))
        .attr("height", (d) => height - y(d.Gasabo));

    svg
        .selectAll(".label.bar.house-gasabo")
        .data(data1)
        .enter()
        .append("text")
        .attr("class", "label bar house-gasabo")
        .attr("x", (d) => x(d.Quarter) + x.bandwidth() / 4)
        .attr("y", height)
        .style("font-size", "15px")
        .style("fill", "black")
        .attr("text-anchor", "middle")
        .attr("y", (d) => y(d.Gasabo) - 5)
        .text((d) => d.Gasabo);
...
fetch('https://rppicalc.onrender.com/IndicesbyRegionapart')
    .then(response => {
        ...
        y.domain([0, d3.max(data2, (d) => d.Gasabo)]);

        // Plot apartment bars and labels
        svg
            .selectAll(".bar.apartment-gasabo")
            .data(data2)
            .enter()
            .append("rect")
            .attr("class", "bar apartment-gasabo")
        ...
        svg
            .selectAll(".label.bar.apartment-gasabo")
            .data(data2)
            .enter()
            .append("text")
            .attr("class", "label bar apartment-gasabo")
